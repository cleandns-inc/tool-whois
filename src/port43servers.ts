...
export const port43parsers: Record<
  string,
...
> = {
  rs(response, record) {
    ...
    (m = response.match(
      /^Registration date: (\d\d)\.(\d\d)\.(\d\d\d\d) (\d\d:\d\d:\d\d)/m
    )) && (record.ts.created = new Date(`${m[3]}-${m[2]}-${m[1]}T${m[4]}Z`));

    (m = response.match(
      /^Modification date: (\d\d)\.(\d\d)\.(\d\d\d\d) (\d\d:\d\d:\d\d)/m
    )) && (record.ts.updated = new Date(`${m[3]}-${m[2]}-${m[1]}T${m[4]}Z`));

    (m = response.match(
      /^Expiration date: (\d\d)\.(\d\d)\.(\d\d\d\d) (\d\d:\d\d:\d\d)/m
    )) && (record.ts.expires = new Date(`${m[3]}-${m[2]}-${m[1]}T${m[4]}Z`));

    (m = response.match(/^(?:Domain status):.*/gm)) &&
      m.forEach((s) => {
        let m;
        (m = s.match(/^Domain status: (\S+)/m)) &&
          record.status.push(normalizeWhoisStatus(m[1]));
      });

    (m = response.match(/^(?:DNS):.*/gm)) &&
      m.forEach((s) => {
        let m;
        (m = s.match(/^DNS: (\S+)/m)) &&
